...
export default function RouteMap() {
  ...
  const filteredAssignments = state.assignments.filter(assignment => {
    ...
    const matchesTruck = !selectedTruck || assignment.truck?.id === selectedTruck;
    const matchesTimeSlot = !selectedTimeSlot || assignment.timeSlot === selectedTimeSlot;
    return matchesDate && matchesTruck && matchesTimeSlot;
  });
  ...
  const availableTrucks = state.trucks.filter(truck => 
    state.assignments.some(a => a.truck?.id === truck.id)
  );
        ...
        assignment.zone.postcodes.forEach((postcode, pcIndex) => {
          ...
          const popup = new mapboxgl.Popup({ offset: 25 }).setHTML(`
            <div style="padding: 8px;">
              ...
              <p style="margin: 0 0 4px 0;"><strong>Truck:</strong> ${assignment.truck?.name || 'Unassigned'}</p>
                ...
                {filteredAssignments.map((assignment, index) => (
                  <div key={assignment.id} className="p-3 rounded border">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-medium">{assignment.truck?.name || 'Unassigned'}</span>
